...
[CreateAssetMenu(menuName = "Flock/Filter/DifferentFlock")]
public class DifferentFlockFilter : ContextFilter
{
    public override List<Transform> Filter(FlockAgent agent, List<Transform> originalNeighbors)
    {
        ...
        foreach(Transform neighbor in originalNeighbors)
        {
            ...
            if (neighborAgent != null && neighborAgent.AgentFlock != agent.AgentFlock)  // If the agent is not in the same flock, add it to the list
